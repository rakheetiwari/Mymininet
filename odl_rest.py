...
def get_all_flow_stats():
    resp, content = h.request('http://192.168.1.1:8080/controller/nb/v2/statistics/default/flow', "GET")
...
def get_all_flows_node(nodeid):
    url = 'http://192.168.1.1:8080/controller/nb/v2/flowprogrammer/default/node/OF/' + nodeid
...
def delete_all_flows_node(node):
    url = 'http://192.168.1.1:8080/controller/nb/v2/flowprogrammer/default/node/OF/' + node
    resp, content = h.request(url, "GET")
    allFlows = json.loads(content)
    flows = allFlows['flowConfig']
    # Dump all flows
...
def delete_spec_flow_node(node, flowname):
    del_url = 'http://192.168.1.1:8080/controller/nb/v2/flowprogrammer/default/node/OF/' + node + '/staticFlow/' + flowname
    logging.debug('del_url %s', del_url)
    resp, content = h.request(del_url, "DELETE")
    logging.debug('resp %s content %s', resp, content)
